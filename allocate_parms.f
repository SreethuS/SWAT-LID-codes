...
!! Variables for soil P and additional operations mjw
	...
	allocate (bmp_bact(mhru))
     ...
     &  wtp_dp(mhyd),wtp_sedi(mhyd),wtp_sede(mhyd),wtp_qi(mhyd))
       allocate (LID_surq_day(mhru))
      
       !MLGA-LID simulations
      allocate (LFcum(mhru,5,nstep),Lsur(mhru,5,nstep),LIDdrain(mhru,5,nstep),Lpav(mhru,5,nstep),
     & WFront(mhru,5,2),Lexfil(mhru,5,nstep),Lsol(mhru,5,nstep),Lstor(mhru,5,nstep),
     & Lsurq(mhru,5,nstep),Lcum_surq(mhru,5),LsucH(msub,5,4),LmSat(msub,5,4),
     & Lksat(msub,5,4),Lthik(msub,5,4),Lthta(msub,5,4),LmIni(msub,5,4),MLGALID_onoff(msub,mudb),
     & LFcum_last(mhru,5),Lsur_last(mhru,5),Ldrain_last(mhru,5),Lexfil_last(mhru,5),
     & Lsurq_last(mhru,5),LID_FB(msub,5),LID_VoidFr(msub,5),LD_drain(msub,5),LIDr_fr(mhru,5),
     & LN_drain(msub,5),LD_height(msub,5),LID_areaFr(msub,5),LR_fr(msub,5),MLID_onoff(msub,5),
     & Lcum_exfil(mhru,5),mlid_farea(mhru,5),LIDthick(mhru,5),Lpav_last(mhru,5),Lstor_last(mhru,5))
...
!!    LID general
	...
	allocate(rspc_a(mhru))	
      ...
      !! ============================
       ...
       !!Allocate for Sreethu's MLGA algorithm for LID
      allocate(C_inf(mhru))
