...
!!    This is the output statement for NRSC      
        do nnj = 1, subtot
            
          write(oflnm, '(I5.5)') nnj
          oflnm1 = 'swatflux_'//trim(mybsn)//'_'//trim(oflnm)//'.dat'
!          write (*,*) oflnm, mybsn, oflnm1     

          !!  If mycde = 1; One continuous new run; Create a new file and rewrite output for all days of simulation
          !!  If mycde = 2; One continuous run to be appended to the already existing output results
          !!  If mycde = 3; Near Real-time run; Append only the last day simulation results to the existing output
          
          select case (mycde)
          case (1)
            if (curyr == 1 .and. iida == id1) then
          open (6001,file= oflnm1, status='replace', action = 'WRITE')
          write(6001, 1111)
1111      format('YYYY MM DD     Pcp_mm Runoff_mm    PET_mm    AET_mm   SEEP_mm     SW_mm   Q_cumec')
          close(6001)
            end if
          
            open (6001,file= oflnm1, access = 'APPEND', action = 'WRITE')

            write (6001,6101) iyr, i_mo, icl(iida), sub_subp(nnj), 
     &      sub_qd(nnj), sub_pet(nnj), sub_etday(nnj), sub_sep(nnj), sub_sw(nnj),
     &      rchdy(2,nnj)
     ...
     &      rchdy(2,nnj)
          ...
          case (3)
!            write (*,*) curyr, nbyr, iida, idal
            if (curyr == nbyr .and. iida == idal) then

            open (6001,file= oflnm, access = 'APPEND', action = 'WRITE')
            write (6001,6101) iyr, i_mo, icl(iida), sub_subp(nnj), 
     &      sub_qd(nnj), sub_pet(nnj), sub_etday(nnj), sub_sep(nnj), sub_sw(nnj),
     &      rchdy(2,nnj)
            close(6001)
...
6101      format (i4,1x,i2,1x,i2,1x,7f10.2)
        enddo
     ...
     &                 wshddayo(6), wshddayo(12) / da_ha, wshddayo(42), 
     &                 wshddayo(45), wshddayo(46), wshddayo(44),        
     &                 wshddayo(40), wshddayo(43), wshddayo(41), wshddayo(114)
        endif
...
6200  format(i5,15f8.2,1x,5f8.2)
      end