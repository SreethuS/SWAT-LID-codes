SUBROUTINE mlga_two(inflow,d,layer1,layer2,F_ini,t,delta_t,dez_t,jL,inf_out,F_out) !t=time(i)
        IMPLICIT NONE
        INTEGER ::jL,ii,iter_num,dez_t
        ...
        REAL*8::T2_1(dez_t) !! for storing time
        REAL*8::A1,B1,delta_t1,ABF !! for layers beneath the first layer
       ...
       !print*,"first layer"
        ...
        ELSEIF ((F_cum>=L1*theta_d1).AND.(F_cum< (L1*theta_d1)+(L2*theta_d2))) then  !wetting front in the second layer
             ! print*," second layer"
            ...
            ELSEIF (ia<=Kbar) then
                ...
                do ii=1,iter_num
                     ...
                     if (((A1+B1+f2(ii))/(ABF)) <=1) then
                            Nr = real(-f2(ii) + FF2 + (Ks2*(t-T2_1(1))))
                        else
        ...
        ELSEIF (F_cum>(L1*theta_d1)+(L2*theta_d2)) then !all layers are saturated
             !print*," alllayer"
