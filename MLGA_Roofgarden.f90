...
subroutine MLGA_RoofGarden (sb,j,k,kk,lid_prec)
   ...
   !WFront(:,:,:)        |hr             |time when wetting front crosses layers
  ...
  !! INTERMEDIATE!!
   ...
   !A_LID                |m2             |area of LID in m2
    ...
    soil_layer,surf_layer


    !!INTERMEDIATE!!
    REAL*8::LRAIN,del_t,delta_t,inf_out,F_out,d,F_ini
    INTEGER::itr,dez_t,iteration,jL,kL !intermediate parameters used in iteration
    REAL*8::tol,D2,D3,theta2,theta3,theta_FC2,theta_FC3,&
    phi2,phi3,q3_old,HCO1,HCO2,omega,x_new(3),x(3),G_new(3),G(3),err_mat(3)!intermediate
    REAL*8::C3D,availEvap,e1,e2,F_next,f2,f3,rate,q3,head,q1,G1,G2,G3,theta_WP3,theta_WP2 !intermediate
  ...
  !! Rainfull input to LID
   ...
   !! layer properties
    ...
    !layer2 = drainage layer
    layer2(1)=200. !suction head in mm
    ...
    !layer3 = roof
    layer3(1)=100 !suction head in mm
    layer3(2)=0.001! moisture content at saturation
    layer3(3)=0.001!initial moisture content
    layer3(4)=0.0001!saturated hydraulic conductivity in mm/hr
    layer3(5)=200!layer thickness in mm
  
    theta_WP2=Lthta(sb,LidN,1) !wilting point in soil layer
    theta_WP3=0.001 !wilting point in storage layer
...
!!computation
    ...
    do itr=1,dez_t,1
        ...
        do iteration=1,100,1
           ...
           if (D_drain==0.0) then
            ...
            elseif (D_drain>0.0) then
                ...
                if ((q3_old==0.0).AND.(head<=h_open)) then
                    q3=0

                end if

                if ((q3_old>0.0).AND.(head<=h_close)) then
                    q3=0
        ...
        exfil=f2
        surf_layer=d
        soil_layer=theta2-theta_WP2

        x=x_new
        G=G_new


    end do   !!discretization
   ...
   Lsurq(j,LidN,k)=(surf_outflow+drain_flow)*del_t*fcimp(urblu(j))*mlid_farea(j,LidN) !runoff from LID
    ...
    Lcum_exfil(j,LidN)=0.!Lcum_exfil(j,LidN) + exfil*del_t*fcimp(urblu(j))*mlid_farea(j,LidN)
    ...
    if (k==nstep)then
        ...
        Lstor_last(j,LidN)=(Lsur(j,LidN,k)+Lsol(j,LidN,k)*layer1(5))*fcimp(urblu(j))*mlid_farea(j,LidN)
