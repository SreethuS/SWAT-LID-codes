...
!!      aird(j) = 0.
      ...
      do j1 = 1, sol_nly(j)
          ...
          if ((MLGALID_onoff(sb,nlid(sb))==1).and.urblu(j)>0)then
              do ii=1,5
                  if ((j1==1).and.(LIDthick(j,ii) <= sol_z(j1,j)))then
                  sepday=sepday + Lcum_exfil(j,ii)
                  elseif (j1>=2) then
                    if ((LIDthick(j,ii)>sol_z(j1-1,j)).and.(LIDthick(j,ii)<= sol_z(j1,j))) then
                     sepday=sepday + Lcum_exfil(j,ii)
                    endif  
                 endif
          end do
