...
!!    adding qtile to output.hru write 3/2/2010 gsm  increased heds(70) to heds(71)
      ...
      character(len=4) :: title(60), cpnm(50000)
...
!!    added for manure Armen Jan 2009
 !!     real*8, dimension (:,:), allocatable :: sol_mc, sol_mn, sol_mp
     ...
     & lid_farea_sum,lid_runfr_sum
      real*8, dimension (:), allocatable ::LID_surq_day
           
      !MLGA LID calculated water balance components and input parameters
      integer, dimension(:,:), allocatable::MLGALID_onoff,MLID_onoff
      real*8,dimension(:,:,:),allocatable:: LFcum,Lsur,
     & LIDdrain,Lexfil,Lsol,Lstor,Lsurq,WFront,LsucH,LmSat,
     & Lksat,Lthik,Lthta,LmIni,Lpav
      real*8, dimension(:,:), allocatable :: LFcum_last,Lsur_last,Lstor_last,
     & Ldrain_last,Lexfil_last,Lsurq_last,Lcum_surq,LID_FB,LIDr_fr,
     & LID_VoidFr,LD_drain,LN_drain,LD_height,LID_areaFr,LR_fr,
     & Lcum_exfil,mlid_farea,LIDthick,Lpav_last!LR_fr- fraction of runoff routed to LIDs
...
!! NRSC, Hyderabad
      character (len=80) :: mybsn
      Integer :: mycde

!!Allocate for Sreethu's MLGA algorithm for LID
 
      real*8, dimension(:), allocatable::C_inf
