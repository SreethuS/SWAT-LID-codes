...
subroutine MLGA_Biocell(sb,j,k,kk,lid_prec)
   ...
   !LFcum_last(:,:,:)    |mm             |cumulative infiltration in sb
   ...
   !LIDdrain             |mm/h           |under-drain flow
  ...
  !! INTERMEDIATE!!
   ...
   !kk                   |none           |lid index in *.mlid files
   !k                    |none           |sub-daily time index
   !sb                   |none           |sub-basin number
   !j                    |none           |HRU number
   ...
   !A_LID                |m2             |area of LID in m2
    ...
    !!INTERMEDIATE!!
    REAL*8::LRAIN,delta_t,inf_out,F_out,d,F_ini,del_t
    INTEGER::itr,dez_t,iteration,jL,kL !intermediate parameters used in iteration
    REAL*8::tol,D2,D3,D4,theta2,theta3,theta4,theta_FC2,theta_FC3,theta_WP2,theta_WP3,&
    phi2,phi3,q3_old,HCO1,HCO2,omega,x_new(3),x(3),G_new(3),G(3),err_mat(3) !intermediate
    REAL*8::C3D,availEvap,e1,e2,e3,F_next,f2,f3,rate,q3,head,q1,G1,G2,G3 !intermediate
    REAL*8::layer1(5),layer2(5),layer3(5)!LID layer properties
...
!!computation
    ...
    do itr=1,dez_t 
        ...
        do iteration=1,100,1
            ...
            CALL mlga_three(LRAIN,d,layer1,layer2,layer3,F_ini,k+itr*delta_t,delta_t,dez_t,jL,kL,inf_out,F_out)
...
Lsurq(j,LidN,k)=(surf_outflow+drain_flow)*del_t*fcimp(urblu(j))*mlid_farea(j,1) !runoff from LID in mm

Lcum_surq(j,LidN)=Lcum_surq(j,LidN)+Lsurq(j,LidN,k) ! cumulative surface runoff in a day in mm
Lcum_exfil(j,LidN)=Lcum_exfil(j,LidN) + exfil*del_t*fcimp(urblu(j))*mlid_farea(j,1) 

!Lcum_surq(sb,LidN)=Lcum_surq(sb,LidN)+Lsurq(sb,LidN,k)*del_t ! cumulative surface runoff
!Lcum_evap(sb,LidN)=Lcum_evap(sb,LidN)+evap(dez_t)*del_t  ! for cumulative evaporation
...
if (k==nstep)then
    ...
    Lstor_last(j,LidN)=(Lsur(j,LidN,k)+Lsol(j,LidN,k)*layer1(5)+Lstor(j,LidN,k)*layer2(5))*fcimp(urblu(j))*mlid_farea(j,LidN)
